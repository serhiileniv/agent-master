...
I keep several projects open at once — that is the whole reason I use Agent Master — so a normal session
...
barely notice them — so it is not that I am running too much, it is that Agent Master tries to start all
of it at once.

## Behaviour

1. When I launch Agent Master with several workspaces saved, every workspace and every agent in them still
