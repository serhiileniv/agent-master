...
Agent Master isn't another agent — it's the surface you run the agents you already pay for on.

## Does Agent Master cost anything to run?

No. Agent Master is MIT-licensed and free, and it has no inference cost of its own — it drives the
agent CLIs already installed on your machine, using your existing credentials and plans.

## Do I need a git repository?

No, but it's recommended. Agent Master opens any folder; a git repo is what unlocks per-agent worktree
...
## Where does Agent Master store my data?

Workspace and layout state lives in `workspaces.json` in the app's user-data folder. Agent
branches live in a sibling `.agentmaster-worktrees/` directory next to your repo. Nothing is sent
...
- **Isolation** — `git worktree add` per agent (branch `canvas/<id>`), kept in a sibling
  `.agentmaster-worktrees/` folder; agents are cwd-pinned after spawn so a shell profile can't move
...
- **Updates** — on launch the app checks the
  [release feed](https://github.com/serhiileniv/agent-master/releases) and shows an in-app notice when
