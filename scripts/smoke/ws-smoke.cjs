...
app.setPath('userData', join(os.tmpdir(), 'agentmaster-ws-ud-' + process.pid))

const A = join(os.tmpdir(), 'agentmaster-ws-A-' + process.pid)
const B = join(os.tmpdir(), 'agentmaster-ws-B-' + process.pid)
...
app.whenReady().then(async () => {
  ...
  const persisted = !!openSet && openSet.includes('agentmaster-ws-A') && openSet.includes('agentmaster-ws-B')
