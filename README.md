<p align="center">
  <img alt="Agent Master" src="build/icon-mac.png" width="148">
</p>
...
running tally in your head of which agent touched what. Agent Master is the place to do it properly:
every agent gets its own pane and its own git worktree, so they never step on each other's
work — or on yours.

## Bring your own agents

Agent Master drives the agent CLIs you already run — **Claude Code, Codex, Gemini, Cursor**, or any
...
> isn't. After dragging Agent Master to Applications, clear the flag once:
>
> ```bash
...
2. **Open a project.** Point Agent Master at any folder; a git repo is what unlocks per-agent isolation.
...
- **[FAQ](docs/FAQ.md)** — cost, git requirements, agent limits, where your data lives, how
  Agent Master compares to tmux and cloud agent platforms
